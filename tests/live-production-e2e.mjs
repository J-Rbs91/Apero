...
async function waitStored(aperoId, predicate, label, timeout = 30000) {
  ...
  while (Date.now() - started < timeout) {
    latest = await fetchStored(aperoId);
...
try {
  ...
  if (await closeLocationSearch.isVisible().catch(() => false)) {
    await closeLocationSearch.click();
    await closeLocationSearch.waitFor({ state: "hidden", timeout: 5000 }).catch(() => {});
  }
  ...
  console.log(`ADMIN_KEY_HASH_PRESENT=${typeof stored.json?.adminKeyHash === "string"}`);
  ...
  check(
    "API VPS déployée est bien la version legacy détectée",
    apiRead.status === 404 && apiReadBody.error === "NOT_FOUND",
  );
  ...
  const afterVote = await waitStored(
    aperoId,
    (value) => Boolean(value && value.sha !== initialSha),
    "mise à jour après vote",
  );
  ...
  await organizer.locator(".person__name", { hasText: GUEST }).waitFor({ state: "visible", timeout: 20000 });
  check("Organisateur relit le vote depuis le stockage réel", true);
  ...
  check(
    "Conflit de version est refusé sans mutation",
    conflict.status === 409 && ["SHA_CONFLICT", "CONFLICT"].includes(conflictBody.error),
  );
  ...
  try {
    await organizer.waitForURL(/#\/agenda/, { timeout: 10000 });
    uiDeleteSucceeded = true;
    check("Annulation UI réussie", true);
  } catch {
    const bodyText = await organizer.locator("body").innerText();
    const relevant = bodyText
      .split("\n")
      .filter((line) => /annul|legacy|suppression|serveur|autor|impossible/i.test(line))
      .slice(-10)
      .join(" | ");
    console.log(`DELETE_UI_RESULT=failed DETAILS=${relevant.slice(0, 1000)}`);
  }

  const finalStored = await fetchStored(aperoId);
  if (uiDeleteSucceeded) {
    check("Fichier supprimé de GitHub après annulation UI", finalStored === null);
  } else {
    check(
      "Échec d'annulation UI n'a pas corrompu le fichier",
      Boolean(finalStored && finalStored.sha === voteSha),
    );
  }

  console.log(
    `RESULT core=PASS aperoId=${aperoId} deleteUi=${uiDeleteSucceeded ? "PASS" : "FAIL_LEGACY_DRIFT"}`,
  );
} catch (error) {
