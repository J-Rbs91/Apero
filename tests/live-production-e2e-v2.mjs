...
try {
  ...
  console.log("STEP organizer reload and synchronization");
  ...
  if (!(await participantsDisclosure.getAttribute("open"))) {
    await participantsDisclosure.locator("summary").click();
  }
  await participantsDisclosure.getByText(GUEST, { exact: true }).waitFor({ state: "visible", timeout: 20000 });
  check("Organisateur relit le vote depuis le stockage réel après reload", true);
  ...
  try {
    await organizer.waitForURL(/#\/agenda/, { timeout: 12000 });
    uiDeleteSucceeded = true;
    check("Annulation UI réussie", true);
  } catch {
    const bodyText = await organizer.locator("body").innerText();
    const relevant = bodyText.split("\n").filter((line) => /annul|legacy|suppression|serveur|autor|impossible|erreur/i.test(line)).slice(-12).join(" | ");
  ...
  console.log(`RESULT core=PASS aperoId=${aperoId} deleteUi=${uiDeleteSucceeded ? "PASS" : "FAIL_LEGACY_DRIFT"}`);
} catch (error) {
